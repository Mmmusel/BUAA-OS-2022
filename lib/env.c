...
static u_int asid_alloc() {
    int i, index, inner;
    for (i = 0; i < 64; ++i) {
        index = i >> 5; //ASID标志的在位图数组的偏移  由于一个int32位，所以位图数组2个int
        inner = i & 31; //ASID标志的在int的偏移
        if ((asid_bitmap[index] & (1 << inner)) == 0) { //1<<inner : i在int是第几个位代表的
            asid_bitmap[index] |= 1 << inner;   //如果当前i位空闲，就占用i个
            return i;   //从小到大申请使用
        }
    }
...
static void asid_free(u_int i) {
    ...
    index = i >> 5;
    inner = i & 31;
    asid_bitmap[index] &= ~(1 << inner);    //释放第i个
}
/* Overview:
...
u_int mkenvid(struct Env *e) {
    u_int idx = e - envs;//idx = e存储在env数组的下标
...
e=envs+ENVX(envid);
    ...
    // if (!(((checkperm!=0)) && ((curenv->env_id == envid) || (e->env_parent_id == curenv->envid)))) {
    //     *penv = 0;///?
    //     return -E_BAD_ENV;
    // }
    if (checkperm!=0) {
        if(!((curenv->env_id == envid) || (e->env_parent_id == curenv->env_id))) {
            *penv = 0;
            return -E_BAD_ENV;
        }
    }
...
{
    int i;
    LIST_INIT(&env_free_list);  //Env_free_list <- env.c(init set)
    LIST_INIT(&env_sched_list[0]);
	LIST_INIT(&env_sched_list[1]);
    struct Env *var = envs + NENV;  //envs <- env.c(init) pmap.c(set)
    var--;
    for (i=NENV;i>0;i--) {   //NENV <- env.h(init set)
        var->env_status = ENV_FREE;
        LIST_INSERT_HEAD(&env_free_list,var,env_link);
        var--;
    }
...
//	printf("alloc begin\n");
    int r;
    struct Env *e;
    if (LIST_EMPTY(&env_free_list)) { *new =NULL;return -E_NO_FREE_ENV;}
                             ...
                             u_char *bin, u_int32_t bin_size, void *user_data)
{
    ...
    Pte *ppte;
    if (offset != 0) {
        if ((p = page_lookup(pgdir,ROUNDDOWN(va, BY2PG),&ppte)) != NULL) {
            bcopy((int *)bin ,(int *)(((u_long)page2kva(p))+offset) , BY2PG-offset);
        } else {
            if ((r = page_alloc(&p)) <0 ) return r;
            bcopy((int *)bin , (int *)(((u_long)page2kva(p))+offset) , BY2PG-offset);
            if ((r = page_insert(pgdir,p,ROUNDDOWN(va, BY2PG),perm))<0) return r; 
        }
        bin+=(BY2PG-offset);
        va = ROUND(va,BY2PG);
	i=BY2PG-offset;
    ...
    for (i  ; (i+BY2PG) <= bin_size; i += BY2PG) {
        if ((r = page_alloc(&p)) <0 ) return r;
        bcopy((int *)bin , (int *)page2kva(p), BY2PG);
        if ((r = page_insert(pgdir,p,va,perm))<0) return r; 
        bin+=BY2PG;
        va+=BY2PG;
    }
    offset = bin_size-i;
    if (offset!=0) {
        if ((p = page_lookup(pgdir,va,&ppte)) == NULL) {
            if ((r = page_alloc(&p)) <0 ) return r;
            if ((r = page_insert(pgdir,p,va,perm))<0) return r; 
        }
        bcopy((int *)bin,(int *)page2kva(p),offset);
        va+=offset;
        i+=offset;
    }
    
    u_long left_offset = BY2PG-offset;
    if (sgsize==bin_size) {return 0;}
    else if((sgsize-bin_size)<=left_offset) {
        bzero((int *)page2kva(p)+offset, (sgsize-bin_size));
        return 0;
    } else {
        bzero((int *)page2kva(p)+offset, left_offset);
        va=ROUND(va,BY2PG);
        i=ROUND(i,BY2PG);
    }
    
    for (i; (i+BY2PG) <= sgsize; i += BY2PG) {
        if ((r = page_alloc(&p)) <0 ) return r;
        if ((r = page_insert(pgdir,p,va,perm))<0) return r; 
        bzero((int *)page2kva(p),BY2PG);
        va+=BY2PG;
    }
    offset =sgsize-i;
    if (offset!=0) {
							 ...
							 u_char *bin, u_int32_t bin_size, void *user_data)
{
    ...
    * i has the value of `bin_size` now. */
	...
	if (offset)
	{
		...
		bzero((void*)(ROUNDDOWN((page2kva(p) + offset),4)), size);
		i = i + size;
	}
	while (i < sgsize) {
		...
		page_insert(env->env_pgdir, p, va + i, PTE_R);
		bzero((void*)page2kva(ROUNDDOWN(p,4)), size);
		i += size;
	}
	return 0;
}
...
//	printf("**************create pri\n");
   ...
   // printf("load icode begin\n");
    load_icode(e,binary,size);
   // printf("load icode done\n");
    LIST_INSERT_HEAD(&(env_sched_list[0]),e,env_sched_link);
   // printf("****create pri done\n");
}
/* Overview:
...
env_free(struct Env *e)
{
   ...
   // printf("[%08x] free env %08x\n", curenv ? curenv->env_id : 0, e->env_id);
    ...
    printf("env free %x\n",e->env_id);/* Hint: free the page directory. */
    pa = e->env_cr3;
    e->env_pgdir = 0;
    e->env_cr3 = 0;
    /* Hint: free the ASID */
...
//	printf("env_run start\n");
	...
	//curenv->env_tf.cp0_epc=KADDR(0x4000b0);
//	printf("epc %x\n",curenv->env_tf.cp0_epc);

    }else {
	    curenv=e;
	   // curenv->env_tf.cp0_epc=curenv->env_tf.pc;
    }
